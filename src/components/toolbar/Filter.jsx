...
import { setBaseUrl } from "../../store/URL/urlSlice";
...
function Filter(){
    ...
    const handleUrl = (e) => {
        let newUrl = 'http://localhost:3000/sorted';

        switch (e.target.id) {
            case 'lowToHigh':
                newUrl += '?sortByPrice=asc';
                break;
            case 'highToLow':
                newUrl += '?sortByPrice=desc';
                break;
            case 'aToZ':
                newUrl += '?sortByName=asc';
                break;
            case 'zToA':
                newUrl += '?sortByName=desc';
                break;
        }
        dispatch(setBaseUrl(newUrl));
    }
    ...
    return(
        ...
        <div className="flex flex-col gap-4">
            ...
            <input id="lowToHigh" type="radio" name="order" onChange={handleUrl}/>
            <label htmlFor="lowToHigh">Low to High</label>
            </span>
            <span className="flex flex-row gap-2">
            <input id="highToLow" type="radio" name="order" onChange={handleUrl}/>
